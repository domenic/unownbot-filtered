...
const happenings = createHappeningsEmitter(stream, config.pokemonDesired, config.accountIDToFollow);
...
happenings.on("desired spawn", data => {
  logger.info(`${data.pokemon} spawned (${data.location} until ${data.until}). Determining distance...`);
});
